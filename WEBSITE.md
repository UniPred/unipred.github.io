...
- `static/js/index.js`: six-phase compact concept flow with continuous automatic playback and manual phase selection, lazy video metadata loading, independent playback of visible clips, global pause/resume, mobile menu, and citation copy.
...
- `static/images/method/observation-top-down.png`: the actual 640×480 workspace-camera image extracted from `imgs/Fig6.pdf`, used in the narrated film.
...
`static/videos/unipred-overview.mp4` is approximately 90 seconds of H.264 video at 1440×810 and 24 fps, with an AAC English narration track. An animated UniPred node mark opens the film. Agent (~5 s), demonstration (~5 s), feedback (~13 s), concept (~5 s), state (~5 s), and planning (~6 s) lead into the complete 46-second `main_demo_1.mp4` recording. There is no four-quadrant assembly. `static/images/agent.svg` is an original small robot icon for the LLM agent; UniPred's network mark represents the learned concept vocabulary. No model name or provider logo is displayed.

The spoken script is `scripts/narration.json`. `static/audio/overview-timing.json` records each measured voice segment and drives both the film and captions; scene durations are not guessed. `static/audio/overview-narration.wav` is the reusable synthesized voice. It was generated locally with the `af_heart` voice in [Kokoro ONNX](https://github.com/thewh1teagle/kokoro-onnx), using its [v1.1 model files](https://github.com/thewh1teagle/kokoro-onnx/releases/tag/model-files-v1.1). This is a synthetic English voice, not a clone of an author or another named person. The website identifies it as AI narration. No background music is included.

To regenerate speech, install `kokoro-onnx` and `soundfile` in an isolated Python environment and download `kokoro-v1.0.fp16.onnx` and `voices-v1.0.bin` from that release (model weights stay outside the repository):

```sh
KOKORO_MODEL=/path/to/kokoro-v1.0.fp16.onnx \
KOKORO_VOICES=/path/to/voices-v1.0.bin python scripts/generate-narration.py
```

Then rerender the film. The committed audio means ordinary video exports do not require the voice model or Python packages. The renderer normalizes narration to −18 LUFS with a −1.5 dB true-peak ceiling, then pads audio for the rest of the complete robot recording.
...
```sh
node scripts/render-overview.cjs --preview  # eight PNG keyframes in /tmp/unipred-site-review/film-v6
...
All visible videos play independently, muted and looping, unless reduced motion or a pause control is active. Native controls remain available. “Watch with narration” is an explicit sound opt-in: the first click restarts the film with audio, and subsequent clicks toggle mute. Clips pause offscreen; manually paused clips stay paused when brought back into view.

The compact method diagram sits directly below the main film. Shared vector primitives in `static/js/concept-visuals.js` connect learning, representation, and planning in one visual strip. On small screens the current part is enlarged on its own. The six phases (Agent, Demonstration, Feedback, Concept, Plan, Action) last 2.6, 3.4, 7.5, 3.6, 4.7, and 5.8 seconds and loop continuously while in view. Phase selection continues playing. Only explicit pause, reduced-motion preferences, an offscreen diagram, or a hidden browser tab stop motion. The Action phase draws the actual source video into the canvas. The global control reports “Motion on/off”; the local control shows the actual playback state rather than a prominent ambiguous “Pause” label. Reduced-motion visitors get a static diagram and can opt into animation.

Concept/predicate nodes are distinct from state nodes: predicates ground observations into facts, and sets of facts describe states. Graph edges are possible action transitions. These drawings and the highlighted plan are illustrative, not measured learning traces or synchronized execution logs. The LLM participates in training; robot inference uses the learned model and planner.
...
The smoke check verifies 11 authors, affiliations and acceptance, local assets and anchors, all 12 visible video files plus the compact action playback, decoded narration audio, opt-in sound and restart, changing robot frames, five immediately visible demonstrations, six-phase autoplay and continuous looping without clicks, phase selection that keeps playing, a pause that freezes canvas motion, simultaneous demos, global motion controls, reduced motion, offscreen pausing, clipboard, and mobile navigation. It checks overflow at 360, 390, 768, and 1440 px and saves screenshots under `/tmp/unipred-site-review/`. Use `SITE_ORIGIN` and `CHROME_PATH` to override defaults.
